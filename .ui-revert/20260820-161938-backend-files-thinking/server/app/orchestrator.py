...
from .providers import Chunk, ContextOverflow, Image, Message, ProviderError, ProviderRouter
...
class Orchestrator:
    ...
    async def run_turn(
        ...
        *,
        ...
        Frames: `meta` (ids, provider, model), `thinking` (reasoning token),
        `delta` (answer token), `done`, `error`.

        `think` is this turn's answer to whether the model should reason first;
        None leaves it to whatever the server was configured with.
        """
        ...
        saved = False
        try:
            async for chunk in self._stream_with_recovery(provider, window, think):
                if chunk.meta.get("kind") == "thinking":
                    # Progress only. Never appended to the stored answer.
    ...
    async def _stream_with_recovery(
        self, provider, window: list[Message], think: bool | None = None
    ) -> AsyncIterator[Chunk]:
    ...
    async def ensure_title(self, session_id: str) -> str | None:
        ...
        # rather than leaving the conversation blank in the drawer.
