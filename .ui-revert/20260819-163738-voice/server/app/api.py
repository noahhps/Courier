...
class TranscribeRequest(BaseModel):
    data: str  # base64 audio, without the data: URL prefix
...
class ChatRequest(BaseModel):
    ...
    # Whether the model should reason before answering. None defers to the
    # server's configured default; the composer's toggle sends a real boolean.
...
def build_router(
    ...
    # Cap on a posted recording. Dictation is a sentence or two; anything past
    # this is a file upload wearing a microphone's clothes.
    ...
    async def transcribe_audio(body: TranscribeRequest) -> dict:
        try:
            ...
            raise HTTPException(400, "That recording could not be read.") from exc

        if len(audio) > MAX_AUDIO_BYTES:
            raise HTTPException(400, "That recording is too long.")

        try:
            text = await transcribe(settings, audio)
    ...
    async def status() -> dict:
        ...
        return {
            ...
            # What the composer's thinking toggle should start as. None means
            # the server has no opinion and the model's own default applies.
            "think": providers.local.think,
        }
