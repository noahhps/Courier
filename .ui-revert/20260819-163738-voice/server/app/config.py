...
class Settings:
    ...
    # --- dictation -------------------------------------------------------
    # Speech to text runs here rather than in the browser, whose recogniser
    # would post the audio to Google or Apple. `small` is the useful floor for
    # dictation and loads in well under a gigabyte; `auto` prefers CUDA and
    # falls back to CPU int8 rather than failing when CTranslate2 can't find
    # its CUDA libraries.
    ...
    whisper_compute: str = field(default_factory=lambda: _env("WHISPER_COMPUTE", ""))
