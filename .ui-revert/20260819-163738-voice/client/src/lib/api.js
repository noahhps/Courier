...
export function createApi(token, onUnauthorized = () => {}) {
  ...
  return {
    ...
    // Recorded audio in, text out. Base64 in JSON like attachments, so the
    // server needs no multipart parser and this takes the same path as
    // everything else -- including the bearer token and the 401 sign-out.
    transcribe: (data, mime) =>
      json("/transcribe", { method: "POST", body: JSON.stringify({ data, mime }) }),
  };
}
